    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            final String[] commandTypeAndParams =
                    split.length == 2 ? split : new String[] {split[0], ""};
            ...
            case "add":
                ...
                if (splitArgs.length == 3 // 3 arguments
                    ...
                    final int indexOfEmailPrefix = commandArgs.indexOf("e/");// name is leading
                                                                             // substring up to
                                                                             // first data prefix
                                                                             // symbol
                    ...
                    final int indexOfEmailPrefix1 = commandArgs.indexOf("e/");// phone is last arg,
                                                                              // target is from
                                                                              // prefix to end of
                                                                              // string
                    if (indexOfPhonePrefix1 > indexOfEmailPrefix1) {
                        result = commandArgs.substring(indexOfPhonePrefix1, commandArgs.length())
                                .trim().replace("p/", "");
                    } else {
                        result = commandArgs.substring(indexOfPhonePrefix1, indexOfEmailPrefix1)
                                .trim().replace("p/", "");
                    }
                    person1[1] = result;
                    String result1;
                    final int indexOfPhonePrefix2 = commandArgs.indexOf("p/");
                    final int indexOfEmailPrefix2 = commandArgs.indexOf("e/");// email is last arg,
                                                                              // target is from
                                                                              // prefix to end of
                                                                              // string
                    if (indexOfEmailPrefix2 > indexOfPhonePrefix2) {
                        result1 = commandArgs.substring(indexOfEmailPrefix2, commandArgs.length())
                                .trim().replace("e/", "");
                    } else {
                        result1 = commandArgs.substring(indexOfEmailPrefix2, indexOfPhonePrefix2)
                                .trim().replace("e/", "");
                    }
                    person1[2] = result1;
                    decodeResult = !person1[0].isEmpty() && !person1[1].isEmpty()
                            && !person1[2].isEmpty() && person1[2].contains("@") ? person1 : null;
                }
                if (decodeResult == null) {
                    feedback = String.format(
                            "Invalid command format: %1$s " + (System.lineSeparator() + "|| ")
                                    + "%2$s",
                            "add",
            ...
            case "list":
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    messageAccumulator.append('\t').append(String.format("%1$d. ", displayIndex))
                            .append(String.format("%1$s  Phone Number: %2$s  Email: %3$s",
            ...
            default:
                feedback = String.format(
                        "Invalid command format: %1$s " + (System.lineSeparator() + "|| ") + "%2$s",
                        commandType,
                        (String.format("%1$s: %2$s", "add", "Adds a person to contacts.")
                                + (System.lineSeparator() + "|| ")
                                + String.format("\tParameters: %1$s",
